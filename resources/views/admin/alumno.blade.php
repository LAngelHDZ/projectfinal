...
<div class="container">
    <div class="d-flex flex-row-reverse my-2  ">
          <a class="btn btn-secondary px-4  text-decoration-none " href="{{ route('registrarAlumno') }}">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-plus-fill mb-1 pe-2" viewBox="0 0 16 16">
        ...
        <table class="table">
          ...
          @endphp
              ...
              <tr>
                ...
                <td class="px-4 bg-light pt-3">{{$item->semestre}}</td>
                <td class="px-3">
                  <a href="{{ route('updateAlumno',$item->id_alumno) }}" class="btn btn-link py-1">
                ...
                <td class="px-4">
                  <form method="POST" action="{{ route('alumnodel') }}">
                    @csrf
                    @method('delete')
                    <input type="hidden" name="idA" value="{{$item->id_alumno}}">
                    <input type="hidden" name="idU" value="{{$item->id}}">
